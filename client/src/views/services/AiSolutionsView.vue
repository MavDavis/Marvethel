  ...
  <div class="pt-20">
    <section class="section-padding border-b border-gray-200 dark:border-white/10">
      <div class="container-wide max-w-4xl">
        <MBadge variant="accent" class="mb-6">AI Solutions</MBadge>
        <h1 class="font-display text-5xl md:text-6xl font-bold text-gray-900 dark:text-white leading-tight mb-6">
          AI features in your product,<br />without a dedicated ML team
        </h1>
        <p class="text-gray-500 dark:text-white/50 text-xl leading-relaxed max-w-2xl mb-10">
          Marvethel gives product teams the infrastructure to build, deploy, and monitor AI features — from custom AI tools and automation workflows to deep analytics and third-party integrations.
        </p>
        <div class="flex gap-4">
          <RouterLink to="/auth/signup"><MButton size="lg">Start Building Free</MButton></RouterLink>
          <RouterLink to="/contact"><MButton variant="outline" size="lg">Talk to an Expert</MButton></RouterLink>
        </div>
      </div>
    </section>
    ...
    <!-- Pricing -->
    <section class="section-padding border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-black">
      <div class="container-wide max-w-4xl">
        <div class="text-center mb-12">
          <MBadge variant="accent" class="mb-4">Pricing</MBadge>
          <h2 class="font-display text-4xl font-bold text-gray-900 dark:text-white">Simple, transparent pricing</h2>
        </div>
        <div class="grid md:grid-cols-3 gap-5">
          <div v-for="tier in tiers" :key="tier.name"
            class="rounded-2xl border p-7 flex flex-col gap-4"
            :class="tier.featured ? 'border-accent/30 bg-lime-50 dark:bg-accent/5' : 'border-gray-200 dark:border-white/10 bg-white dark:bg-white/[0.02]'">
            <div>
